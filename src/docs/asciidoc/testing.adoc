...
The Spring MVC test.Test framework builds on the mock Servlet API objects to provide an
...
<<spring-mvc-test-framework, Spring MVC test.Test Framework>> instead.
...
test.Test classes typically declare either an array of resource locations for XML or Groovy
...
[[testing-fixture-di]]
==== Dependency Injection of test.Test Fixtures
...
Spring's `@Timed` annotation has different semantics than JUnit 4's `@test.Test(timeout=...)`
support. Specifically, due to the manner in which JUnit 4 handles test execution timeouts
(that is, by executing the test method in a separate `Thread`), `@test.Test(timeout=...)`
...
Since JUnit Jupiter supports the use of `@test.Test`, `@RepeatedTest`, `ParameterizedTest`,
...
the `@test.Test` and `@Tag` annotations from JUnit Jupiter with the `@Transactional`
...
----
	...
	@test.Test // org.junit.jupiter.api.test.Test
...
* test.Test instance post-processing.
...
for which it is responsible. test.Test instances do not automatically receive access to the
...
test.Test classes that use the TestContext framework do not need to extend any particular
...
[[testcontext-ctx-management-property-sources]]
===== Context Configuration with test.Test Property Sources
...
====== Declaring test.Test Property Sources
...
test.Test properties have higher precedence than those defined in the operating system's
...
====== Inheriting and Overriding test.Test Property Sources
...
.test.Test suites and forked processes
[NOTE]
====
...
===== test.Test-managed Transactions

test.Test-managed transactions are transactions that are managed declaratively by using the
...
hierarchy runs within a transaction. test.Test methods that are not annotated with
...
----
	...
	@Transactional
	@test.Test // no expected exception!
...
`@BeforeAll`, `@AfterAll`, `@BeforeEach`, `@AfterEach`, `@test.Test`, `@RepeatedTest`,
...
[[spring-mvc-test-framework]]
=== Spring MVC test.Test Framework

The Spring MVC test.Test framework provides first class support for testing Spring MVC code
...
Spring MVC test.Test also provides client-side support for testing code that uses the
...
The goal of Spring MVC test.Test is to provide an effective way to test controllers by
performing requests and generating responses through the actual `DispatcherServlet`.

Spring MVC test.Test builds on the familiar <<mock-objects-servlet, "`mock`" implementations of
...
Jupiter-based example uses Spring MVC test.Test:
...
In Spring MVC test.Test, async requests can be tested by asserting the produced async value
...
There are no options built into Spring MVC test.Test for container-less testing of streaming
...
===== Spring MVC test.Test vs End-to-End Tests

Spring MVC test.Test is built on Servlet API mock implementations from the
...
There are pros and cons for each approach. The options provided in Spring MVC test.Test are
different stops on the scale from classic unit testing to full integration testing. To be
certain, none of the options in Spring MVC test.Test fall under the category of classic unit
...
Another important distinction when using Spring MVC test.Test is that, conceptually, such
...
sample tests] intended to show how to use Spring MVC test.Test. You can browse these examples
for further ideas. Also, the
https://github.com/spring-projects/spring-mvc-showcase[`spring-mvc-showcase`] project has
full test coverage based on Spring MVC test.Test.
...
* test.Test JavaScript within pages.
...
With Spring MVC test.Test, we can easily test if we are able to create a `Message`, as follows:
...
To use Selenium WebDriver with the Spring MVC test.Test framework, make sure that your project
...
----
	...
	// test.Test code that uses the above RestTemplate ...
...
Spring MVC test.Test's own tests include
...
* https://mockito.github.io[Mockito]: Java mock library based on the
  http://xunitpatterns.com/test.Test%20Spy.html[test.Test Spy] pattern. Used by the Spring Framework
  in its test suite.
