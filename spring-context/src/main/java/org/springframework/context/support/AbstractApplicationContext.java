...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			 ...
			 * 2.设置容器活跃状态为true；
			 * 3.设置容器关闭状态为false；
			 * 4.获取Environment对象，并把当前系统的属性值加载到Environment对象中；
			...
			try {
				...
				// 在所有注册的bean中查找的listener bean，注册到消息广播器中
				registerListeners();
	...
	protected void prepareRefresh() {
		...
		// 判读刷新前的应用程序监听器集合是否为空，如果为空，则将监听器添加到此集合中
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 这是要忽略自动装配的接口（忽略的原因是因为这些接口的实现是由容器通过set方法进行注入的）
		...
		// 增加对AspectJ的支持，在java中织入分为三种方式，分为编译器织入，类加载器织入，运行期织入，编译期织入是指在java编译器，采用特殊的编译器，将切面织入到java类中。
		// 而类加载期织入则指通过特殊的类加载器，在类字节码加载到JVM时，织入切面，运行期织入则是采用cglib和jdk进行切面的织入
		// aspectj提供了两种织入方式，第一种是通过特殊编译器，在编译器将aspectj语言编写的切面类织入到java类中，第二种是类加载期织入，就是下面的load time weaving.
